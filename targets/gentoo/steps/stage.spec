...
elif [ "$pf" = "slontoo" ]; then
	path="gentoo:"
	path="$[profile/path:zap]/"
	install -d /etc/portage/make.profile
	cat > /etc/portage/make.profile/parent << EOF
${path}arch/$[profile/arch:zap]
...
sync-type = $[snapshot/source/type]
sync-uri = $[snapshot/source/remote]
auto-sync = yes
