...
class MetroSetup(object):
	...
	def getSettings(self, args={}, extraargs=None):
		...
		if settings.has_key("path/mirror/tarout") and not settings.has_key("path/mirror/snapshot"):
			snapshot_name = settings["path/mirror/tarout"]
			if settings.has_key("snapshot/compression"):
				snapshot_compression = settings["snapshot/compression"]
				if snapshot_compression != "none":
					snapshot_name += "." + snapshot_compression
			settings["path/mirror/snapshot"] = snapshot_name
